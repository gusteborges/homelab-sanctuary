...
const RemoteAccessPage = () => {
  ...
  const fetchMachines = async () => {
    try {
      ...
      console.error("Erro ao carregar máquinas remotas:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchMachines();
    const interval = setInterval(fetchMachines, 30000); // Atualiza status a cada 30s
    return () => clearInterval(interval);
  }, [token]);

  // Função para enviar o Magic Packet (Wake-on-LAN)
  const handleWake = async (mac: string, name: string) => {
    ...
    try {
      ...
      if (response.ok) {
        alert(`Comando Wake enviado para ${name}!`);
      }
    } catch (err) {
      alert("Erro ao enviar comando de inicialização.");
    } finally {
      setWaking(null);
    }
  };

  const gamingPC = machines.find(m => m.is_gaming_pc) || machines[0];
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Controle sua infraestrutura de onde estiver</p>
        </div>
          ...
          <CardContent className="space-y-4">
            <div className="p-3 rounded-md bg-muted/30 border border-border/50 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-mono text-foreground">{gamingPC?.name} ({gamingPC?.ip_address})</span>
              </div>
              <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button 
                ...
                onClick={() => window.open(`moonlight://`)} // Tenta abrir o cliente local
              >
              ...
              <Button 
                ...
                disabled={gamingPC?.is_online || waking === gamingPC?.name}
              >
          ...
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>Acesso via RDP/VNC/SSH</CardDescription>
              </div>
            </div>
          </CardHeader>
